#!/usr/bin/env python3
"""
Comedy Clipper - FFmpeg version (most reliable)
Automatically clips videos at scene changes using FFmpeg directly
"""
...
class FFmpegComedyClipper:
    """Clipper using pure ffmpeg - most reliable approach"""
    ...
    def clip_video_ffmpeg(self, video_path: str, start: float, end: float, output_path: str):
        """
        Clip video using ffmpeg directly (fast and reliable).
...
def main():
    parser = argparse.ArgumentParser(
        description="Comedy clipper using FFmpeg scene detection",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage
  python clipper_ffmpeg.py standup_show.mp4

  # Custom sensitivity
  python clipper_ffmpeg.py show.mp4 --threshold 0.4

  # Custom output directory
  python clipper_ffmpeg.py show.mp4 -o clips/

  # Adjust minimum duration
  python clipper_ffmpeg.py show.mp4 -m 60
        """
    )
    ...
    try:
        clipper = FFmpegComedyClipper(scene_threshold=args.threshold)
