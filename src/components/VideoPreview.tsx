import { Video, Play, Scissors, FolderOpen } from 'lucide-react'
...
const VideoPreview: React.FC<VideoPreviewProps> = ({
  ...
  const videoUrl = videoPath ? ((window as any).electron?.getLocalFileUrl(videoPath) || videoPath) : null
    ...
    <div className="flex-1 flex flex-col bg-slate-900 overflow-hidden">
      <div className="flex items-center justify-between p-6 border-b border-slate-700">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">{videoPath ? videoPath.split('/').pop() : 'No video selected'}</p>
            {segments.length > 0 && (
        ...
        <div className="flex items-center gap-3">
          {onSelectNewVideo && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-3 bg-slate-700 hover:bg-slate-600 disabled:bg-slate-800 disabled:text-slate-600 rounded-lg font-medium transition-colors"
            >
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-black">
        {videoPath ? (
          ...
          <div className="flex flex-col items-center justify-center gap-4 text-slate-400">
            ...
            <p className="text-sm">Click "Select New Video" to get started</p>
          </div>
        )}

        {/* Custom video controls with segment markers */}
        {videoPath && <div className="w-full max-w-4xl mt-6 space-y-3">
      ...
      <div className="p-4 border-t border-slate-700 bg-slate-800">
        <div className="text-sm text-slate-400">
          <p className="font-mono text-xs truncate">{videoPath || 'No video selected'}</p>
          <p className="mt-2 text-xs">
            {videoPath
              ? 'Preview your video before processing. Adjust settings in the sidebar, then click "Start Clipping" to begin.'
              : 'Select a video to get started with the comedy clipper.'}
          </p>
        </div>
      </div>
    </div>
