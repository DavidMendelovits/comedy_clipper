#!/usr/bin/env python3
"""
Comedy Clipper - MediaPipe Pose Detection
Detects when a person enters/exits the stage using MediaPipe pose landmarks
More accurate than YOLO bounding boxes for tracking body position
"""
...
class MediaPipePoseClipper:
    """Clipper using MediaPipe pose detection to identify entry/exit from stage"""
    ...
    def export_debug_frame(self, frame, frame_num: int, center_x: Optional[float],
                          ...
                          output_path: str, label: str, landmarks=None):
        """
        ...
        Args:
            ...
            landmarks: MediaPipe pose landmarks (optional)
        """
    ...
    def detect_person_segments(self, video_path: str,
        ...
        """
        Detect when a person exits stage left or right using pose landmarks.
        ...
        """
        print(f"Analyzing video for stage exits using MediaPipe Pose...")
        ...
        person_positions = []
        # Store landmarks for debug export
        debug_landmarks = {}
        ...
        while True:
            ...
            if frame_num % sample_interval == 0:
                ...
                if results.pose_landmarks:
                    ...
                    # This is more stable than using a single point
                    ...
                    # Calculate center x position (normalized 0-1, convert to pixels)
                    torso_x = (left_shoulder.x + right_shoulder.x + left_hip.x + right_hip.x) / 4
                    person_center_x = torso_x * frame_width
                    ...
                    # Store landmarks for debug export
                    if self.debug:
                        debug_landmarks[frame_num] = results.pose_landmarks
        ...
        if self.debug and segments and segment_frames:
            ...
            for seg_idx, frames_data in enumerate(segment_frames[:len(segments)], 1):
                ...
                if ret:
                    ...
                    landmarks = debug_landmarks.get(last_frame_num)
                    self.export_debug_frame(frame, last_frame_num, last_x, last_edge,
                                          left_edge, right_edge, output_path,
                                          f"Segment {seg_idx} - LAST", landmarks)
    ...
    def clip_video(self, video_path: str, segments: List[Tuple[float, float]],
        ...
        output_dir = base_dir / f"{video_name}_clips_mediapipe_{timestamp}"
    ...
    def process_video(self, video_path: str, output_dir: str = None,
        ...
        segments = self.detect_person_segments(video_path, exit_threshold)
...
def main():
    parser = argparse.ArgumentParser(
        description="Comedy clipper using MediaPipe pose detection (detects entry/exit from stage)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage (3 min minimum)
  python3 clipper_mediapipe.py standup_show.mp4

  # 5 minute minimum sets
  python3 clipper_mediapipe.py show.mp4 -m 300

  # Save to specific directory
  python3 clipper_mediapipe.py show.mp4 -o clips/

  # Enable debug mode (exports first/last frames with overlays)
  python3 clipper_mediapipe.py show.mp4 -d

How it works:
  - Uses MediaPipe to detect 33 body landmarks for accurate pose tracking
  - Tracks torso center (average of shoulders and hips) for stable position
  - Detects when person exits stage left or right based on torso position
  - More accurate than bounding boxes for tracking body position
  - Perfect for static camera standup shows with comedians exiting stage left/right
    ...
    try:
        clipper = MediaPipePoseClipper(debug=args.debug)
